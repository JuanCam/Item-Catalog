# Catalog application for working with the CRUD of the Items and catories
...
>>>>>>> c794008843811ae07c9ec91247304eab066f3e0e
...
# Disconnect Gmail account decorator
...
def showLogin():
	...
	sports_items = session.query(CategoryTable.name,CategoryTable.id,ItemTable.name).\
				join(ItemTable.category).group_by(CategoryTable.id).\
	...
	# The flag variable button_rows shows or hide the Create Item/Category buttons.
...
def home():
	"""This method set up the whole home view once a user is logged in. - not public"""
	...
	sports_items = session.query(CategoryTable.name,CategoryTable.id,ItemTable.name).\
		...
		order_by(CategoryTable.name).all()
	# If ther's no name field in the login_session object, redirect to the login page
...
# Filter Item decorator
@appCatalog.route('/filter-items/<int:idcategory>')
def filterItem(idcategory):
	"""This is filter item method for rendering the items-per-category view. - public"""

	# Select all the items related to the selected category
	sport_items = session.query(CategoryTable.name,ItemTable.id,ItemTable.name).\
		join(ItemTable.category).\
	...
	name = ''
	image = ''
	# Flag that determines whether the edit and delete options are shown or not.
	show_rows = True
...
def editItem(iditem):
	"""Edit Item view, it allows the user to change the item information - not public"""
	# Get the item
...
def createItem():
	"""View for creating a new Item - not public"""
	if request.method=='POST':
		item_name = request.form['item_name']
		id_c = request.form['category']
		description = request.form['description']
...
def viewItem(iditem):
	...
	return render_template('view-item.html',
                            ...
                            image=image)
# JSON View decorator
@appCatalog.route('/filter-items/<int:idcategory>/JSON')
def homeJSON(idcategory):
	"""Shows the JSON Format for all the items of a category"""

	sports_items = session.query(ItemTable).filter_by(category_id = idcategory).all()
	return jsonify(Categories=[s.serial for s in sports_items])
